...
class RiskManager:
    ...
    def calculate_stop_loss(self, symbol, side, entry_price):
        ...
        # Special handling for LAYER tokens which have higher volatility
        is_layer = 'LAYER' in symbol
        ...
        # For LAYER tokens, add more buffer to the stop loss
        if is_layer:
            original_pct = stop_loss_pct
            stop_loss_pct = stop_loss_pct * 1.5  # 50% wider stops for LAYER tokens
            logger.info(f"LAYER token detected: Increasing stop loss percentage from {original_pct*100:.2f}% to {stop_loss_pct*100:.2f}%")
        ...
        if is_layer:
            logger.info(f"Calculated LAYER-specific {self.current_market_condition} stop loss at {stop_price} ({stop_loss_pct*100:.2f}%, enhanced buffer active)")
        else:
    ...
    def calculate_volatility_based_stop_loss(self, symbol, side, entry_price, klines=None):
        ...
        try:
            ...
            if len(df) >= atr_period:
                ...
                # Apply LAYER-specific adjustments - increase multiplier by 50%
                if is_layer:
                    original_multiplier = atr_multiplier
                    atr_multiplier = atr_multiplier * 1.5
                    logger.info(f"LAYER token detected: Increasing ATR multiplier from {original_multiplier} to {atr_multiplier}")
                ...
                if side == "BUY":  # Long
                    ...
                    # For LAYER, increase the maximum stop distance by 50%
                    if is_layer:
                ...
                # Add LAYER-specific buffer information to log
                if is_layer:
                    logger.info(f"Calculated LAYER-specific ATR-based stop loss at {stop_price} "
